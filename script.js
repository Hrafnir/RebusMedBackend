...
async function loginTeam(teamName, password) {
    ...
    const email = teamName.toUpperCase() + FICTITIOUS_DOMAIN;
        ...
        if (hasLoadedTeamData && currentTeamData) {
            ...
            if (currentTeamData.completedPostsCount >= TOTAL_POSTS) {
                ...
                if (currentTeamData.postSequence && currentPostArrayIndex < currentTeamData.postSequence.length) {
                    ...
                    console.warn("Ugyldig postsekvens eller indeks i lastet data. Viser finalen.");
                    showRebusPage('finale-page'); 
                    if (map) updateMapMarker(null, true);
                }
            }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.unlock-post-btn').forEach(button => { button.addEventListener('click', async () => { const postNum = button.getAttribute('data-post'); const input = document.getElementById(`post-${postNum}-unlock-input`); if (input) await handlePostUnlock(postNum, input.value.trim().toUpperCase()); }); });
    document.querySelectorAll('.check-task-btn').forEach(button => { button.addEventListener('click', async () => { const postNum = button.getAttribute('data-post'); const input = document.getElementById(`post-${postNum}-task-input`); if (input) await handleTaskCheck(postNum, input.value.trim().toUpperCase()); }); });
